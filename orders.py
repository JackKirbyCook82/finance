...
class OrderCalculator(Separating, Sizing, Emptying, Logging):
    ...
    def orders(self, prospects, *args, **kwargs):
        ...
        for index, series in prospects.iterrows():
            stocks = self.securities(series, *args, columns=list(map(str, Categories.Securities.Stocks)), **kwargs)
            options = self.securities(series, *args, columns=list(map(str, Categories.Securities.Options)), **kwargs)
    ...
    def securities(prospects, *args, columns, **kwargs):
        ...
        function = lambda cols: list(Categories.Securities(cols["security"])) + [1]
