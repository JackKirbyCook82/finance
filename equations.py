...
@njit(cache=True, inline="always")
def error(y, x, k, r, σ, τ, i): return value(x, k, r, σ, τ, i) - y

@njit(cache=True, inline="always")
def intrinsic(x, k, r, τ, i):
    dcf = discount(r, τ)
...
@njit(cache=True, inline="always")
def brenner(y, x, k, r, τ, i, /, low, high):
    yτ = intrinsic(x, k, r, τ, i)
    dy = max(y - yτ, 1e-12)
    x = max(x, 1e-12)
...
@njit(cache=True)
def newton(y, x, k, r, σ, τ, i, /, low, high, tol, iters):
    σ = brenner(y, x, k, r, τ, i, low, high)
    for _ in range(iters):
        err = value(x, k, r, σ, τ, i) - y
...
@njit(cache=True)
def bisection(y, x, k, r, τ, i, /, low, high, tol, iter):
    yl = error(y, x, k, r, low, τ, i)
    yh = error(y, x, k, r, high, τ, i)
...
@njit(cache=True)
def fitted(y, x, k, r, τ, i, /, low, high, tol, iters):
    assert i == 1 or i == -1
    σl, σh = low, high
    for _ in range(iters):
        σml = σl + (σh - σl) / 3.0
        σmh = σh - (σh - σl) / 3.0
        yml = abs(value(x, k, r, σml, τ, i) - y)
        ymh = abs(value(x, k, r, σmh, τ, i) - y)
        if yml < ymh: σh = σmh
        else: σl = σml
        if (σh - σl) < tol: break
    return 0.5 * (σl + σh)
...
@njit(cache=True)
def volatility(y, x, k, r, τ, i, /, low, high, tol, iters):
    ...
    if σh <= low: σh = high
    σ = newton(y, x, k, r, τ, i, low, σh, tol, iters)
    if math.isfinite(σ): return σ
    return bisection(y, x, k, r, τ, i, low, σh, tol, max(100, iters))

@njit(cache=True)
def residual(y, x, k, r, τ, i):
...
@njit(cache=True)
def implied_strict(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    return volatility(y, x, k, r, τ, i, low=low, high=high, tol=tol, iters=iters)

@njit(cache=True)
def implied_clipped(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    if not valid(x, k, τ, i) or low <= 0.0 or high <= low: return math.nan
    yl, yh = boundary(x, k, r, τ, i)
    σh = min(high, adaptive(x, k, τ))
    if y < yl - tol: return low
    if y > yh + tol: return σh
    σ = volatility(y, x, k, r, τ, i, low=low, high=σh, tol=tol, iters=iters)
    if math.isfinite(σ): return σ
    return low if (y - yl) <= (yh - y) else σh

@njit(cache=True)
def implied_fitted(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    if not valid(x, k, τ, i) or low <= 0.0 or high <= low: return math.nan
    σh = min(high, adaptive(x, k, τ))
    σ = volatility(y, x, k, r, τ, i, low=low, high=σh, tol=tol, iters=iters)
    if math.isfinite(σ): return σ
    return fitted(y, x, k, r, τ, i, low, σh, tol, iters)

@njit(cache=True)
def residual_strict(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    return residual(y, x, k, r, τ, i)

@njit(cache=True)
def residual_clipped(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    return residual(y, x, k, r, τ, i)

@njit(cache=True)
def residual_fitted(y, x, k, r, τ, i, low=1e-4, high=5.0, tol=1e-10, iters=100):
    if not valid(x, k, τ, i) or low <= 0.0 or high <= low: return math.nan
    σ = implied_fitted(y, x, k, r, τ, i, low=low, high=high, tol=tol, iters=iters)
    if not math.isfinite(σ): return math.nan
    return value(x, k, r, σ, τ, i) - y


class Equations(Assembly):
    class Greeks(Assembly): Delta, Gamma, Theta, Rho, Vega, Vomma, Vanna, Charm = delta, gamma, theta, rho, vega, vomma, vanna, charm
    class Volatility(Assembly):
        class Strict(Assembly): Implied, Residual = implied_strict, residual_strict
        class Clipped(Assembly): Implied, Residual = implied_clipped, residual_clipped
        class Fitted(Assembly): Implied, Residual = implied_fitted, residual_fitted
