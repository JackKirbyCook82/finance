...
class MarketCalculator(Sizing, Emptying, Partition, Logging, ABC, title="Calculated"):
    ...
    @staticmethod
    @abstractmethod
    def supply(securities, *args, **kwargs): pass
    @abstractmethod
    def calculate(self, valuations, options, *args, **kwargs): pass
...
class DivestitureCalculator(MarketCalculator):
    ...
    def supply(securities, *args, **kwargs):
        security = lambda cols: str(Securities([cols["instrument"], cols["option"], cols["position"]]))
        header = list(Querys.Settlement) + list(Variables.Securities.Security) + ["strike", "size", "quantity"]
        supply = securities[header]
        supply["security"] = supply.apply(security, axis=1)
        index = list(Querys.Settlement) + ["security", "strike"]
        supply = supply[index + ["size", "quantity"]].set_index(index, drop=True, inplace=False)
        return supply
